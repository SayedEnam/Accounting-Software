...
class ProductServiceCategoryController extends Controller
{
    ...
    public function create()
    {
        if (\Auth::user()->can('create constant category')) {
            $types = \App\Models\ProductServiceCategory::$catTypes; // Added full namespace
            $type = ['' => 'Select Category Type'];
            $types = array_merge($type, $types);

            // This part of your code is fine
            $chart_accounts = \App\Models\ChartOfAccount::select(\DB::raw('CONCAT(code, " - ", name) AS code_name, id'))
            ...
            // ✨ FIX: Return the error view instead of JSON
            return view('partials.error', ['error' => __('Permission denied.')]);
        }
    }
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('create constant category')) {
            ...
            $validator = \Validator::make(
                $request->all(),
                [
                    'name' => 'required|max:200', // Increased max from 20 to 200 to be safe
                    'type' => 'required',
                    'color' => 'required',
                ]
            );

            // --- Step 1: Handle Validation Failure ---
            if ($validator->fails()) {
                $messages = $validator->getMessageBag();

                // Check if the request is AJAX
                if ($request->ajax()) {
                    // If it is, return a JSON response with errors
                    return response()->json([
                        'success' => false,
                        'errors' => $messages
                    ], 422);
                } else {
                    // If it's a regular request, redirect back with an error message
                    return redirect()->back()->with('error', $messages->first());
                }
            }

            // --- Step 2: Save the data (this logic is the same for both) ---
            ...
            // Check if the request is AJAX
            if ($request->ajax()) {
                // If it is, return a JSON response with the new category
                return response()->json([
                    'success' => true,
                ...
                // If it's a regular request, redirect to the index page with a success message
                return redirect()->route('product-category.index')->with('success', __('Category successfully created.'));
            }
        } else {
            // Handle permission denied for both request types
            if ($request->ajax()) {
    ...
    //         } elseif (route('product-category.index')) {
    //             return redirect()->route('product-category.index')->with('success', __('Category successfully created.'));
    //         } else {
    //             return redirect()->back()->with('error', __('Permission denied.'));
    //         }
